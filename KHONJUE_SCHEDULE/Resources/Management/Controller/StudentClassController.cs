...
namespace KHONJUE_SCHEDULE.Resources.Management.Controller
{
    public class StudentClassController
    {
        ...
        public List<StudentClassModel> GetAvailableRooms(string day, int periodId)
        {
            try
            {
                List<StudentClassModel> classes = new List<StudentClassModel>();
                _command = new NpgsqlCommand();
                _command.Connection = _databaseContext.dbConnection;
                _command.CommandText = $@"SELECT student_class.""Id"",student_class.""Code"", student_class.""StudentClassName"", student_class.""Description"",student_class.""NumberOfClass"",student_class.""RoomType""  FROM public.student_class  WHERE NOT EXISTS (
        SELECT 1 
        FROM schedule s 
        WHERE s.""RoomId"" = student_class.""Id"" AND s.""Day"" = '{day}' AND s.""TimePeriodId"" = {periodId}
    );";
                NpgsqlDataReader data = _command.ExecuteReader();
                while (data.Read())
                {
